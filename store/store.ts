...
class Store {
  ...
  async addUsers(db: SQLiteDatabase, users: User[]) {
    try {
      if (!_.isEmpty(users)) {
        ...
        const values = _.flatMap(users, (user) => [
          user.address,
          user.displayName || "",
          user.publicKey,
        ]);
  ...
  async addMessages(db: SQLiteDatabase, messages: Message[], admin: string) {
    try {
      if (!_.isEmpty(messages)) {
        ...
        const placeholders = Array(messages.length)
          .fill("(?, ?, ?, ?, ?)")
          .join(",");
        const values = _.flatMap(messages, (message) => [
          message.id,
          message.sender,
          message.receiver,
          message.content,
  ...
  async removeMessage(message: Message, addressUser: string) {
    this.middleWare(
      () =>
        (this.messages[addressUser] = _.without(
          this.messages[addressUser],
          message
        ))
    );
  }
